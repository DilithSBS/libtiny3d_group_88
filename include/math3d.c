/*
    All the functions related to mathematical concepts and logics are implemented here
*/
...
vec3_t vec3_normalize_fast(vec3_t v) {
    ...
    // Getting the squared length
    float lenSq = v.x * v.x + v.y * v.y + v.z * v.z;
    float invLen = invSqRoot(lenSq);        // Inverse length

    // Multiply each component by the inverse length to normalize the vector
    v.x *= invLen;
    v.y *= invLen;
    v.z *= invLen;
...
vec3_t vec3_slerp(vec3_t a, vec3_t b, double t) {
    ...
    double sinAlpha = sin(alpha);        // sin of the angle
    ...
    // retunn the final interpolated 3D vector after computing weighted component
    vec3_t v;
    v.x = w1 * a.x + w2 * b.x;
    v.y = w1 * a.y + w2 * b.y;
    v.z = w1 * a.z + w2 * b.z;
...
mat4 mat4_rotate_xyz(float rx, float ry, float rz) {
    ...
    // calculating sin and cosin values
    ...
    // Return the full rotation around all axes
    return mat4_mul(rzm, mat4_mul(rym, rxm));
}

// create a 4×4 perspective projection matrix
...
quat_t quat_from_axis_angle(vec3_t axis, float angle_rad) {
    float half = angle_rad * 0.5f;      // Calculate the half angle
    float sin_half = sinf(half);        // Sin of the half angle
    return (quat_t){
        .w = cosf(half),            // Scalar part
        .x = axis.x * sin_half,     // X component
        .y = axis.y * sin_half,     // Y component
        .z = axis.z * sin_half      // Z component
    };
}
...
quat_t quat_slerp(quat_t a, quat_t b, float t) {
    ...
    // If the angle is very small, use linear interpolation for stability
    if (dot > 0.9995f) {
        quat_t result = {
            a.w + t*(b.w - a.w),
            a.x + t*(b.x - a.x),
